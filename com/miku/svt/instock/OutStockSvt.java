...
import com.miku.dao.buy.OutStockDao;
 ...
 * @description:
 */
@WebServlet("/OutStockSvt")
public class OutStockSvt extends HttpServlet {
    private SupplierDao sd = new SupplierDao();
    private ProductDao pd = new ProductDao();
    private SplitePage sp = new SplitePage();
    private OutStockDao osd = new OutStockDao();
    //本次进货商品集合
    private ArrayList<OutStockProInfo> saleStockPros = new ArrayList<>();
    ...
    private void addInStock(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        if (saleStockPros.isEmpty()) {
            ...
            OutStockInfo ssi = new OutStockInfo();
            try {
                ssi.setOutStockId(inStockId);
                //供应商id
                ssi.setSupplierId(Integer.valueOf(supplier));
                ssi.setSupplierName(sd.querySupplierById(Integer.valueOf(supplier)).getSupplierName());
            ...
            //生成进货单
            int row = osd.addOutStock(ssi);
            if (row > 0) {
                //添加商品
                int row2 = 0;
                for (OutStockProInfo sspi : saleStockPros) {
                    //添加退货单商品
                    row2 += osd.addInStockPro(sspi);
                    //更改库存数量
                    ProductInfo pro = pd.queryProByProId(sspi.getProId());
                    //库存数量等于原数量-退货数量
                    pro.setProNum(pro.getProNum() - sspi.getProNum());
                    //更改商品售价
//                    pro.setOutPrice(sspi.getInPrice());
                    //修改状态至2表示该商品已经产生订单
                    pro.setState("2");
                    pd.alterPro(pro);
...
//                json.setObj(saleStockPros);
            ...
            OutStockProInfo sspi = new OutStockProInfo();
            //售价
            sspi.setInPrice(inPrice);
            sspi.setProNum(proNum);
            //出库订单号
            sspi.setInStockId(inStockId);
    ...
    private void start2(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        String saleStockid = "th0000000000";
        Date d = new Date();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
        String no = "th" + sdf.format(d);
        ArrayList<String> inStockIds = osd.queryAllInStockId();
