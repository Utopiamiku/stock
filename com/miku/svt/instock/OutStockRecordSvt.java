 ...
 * @description:
 */
@WebServlet("/OutStockRecordSvt")
public class OutStockRecordSvt extends HttpServlet {
    private SplitePage sp1 = new SplitePage();

    private OutStockDao ssd = new OutStockDao();
    private SupplierDao cd = new SupplierDao();

    private ArrayList<OutStockInfo> list = new ArrayList<>();
    ...
    private void queryInStock(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (!inStockId.equals("abc")) {
            OutStockInfo ssi = ssd.queryOutStockById(inStockId);
            ArrayList<OutStockInfo> list1 = new ArrayList<>();
            list1.add(ssi);
    ...
    private void deleteStock(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        ArrayList<OutStockProInfo> ispi = ssd.queryProByInStockId(inStockId);

        int row = ssd.deleteOutStock(inStockId);
        JsonResult json = new JsonResult();
    ...
    private void start2(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ArrayList<SupplierInfo> clientInfos = cd.queryAllSupplier();
    ...
    private void queryDetail(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        String inStockId = req.getParameter("inStockId");
        ArrayList<OutStockProInfo> list = ssd.queryProByInStockId(inStockId);
    ...
    private void start1(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        list = ssd.queryInStockByPage(sp1);
        sp1.setT(list);
