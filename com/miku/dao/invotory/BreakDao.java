...
/**
 * @author: Utopiamiku
 * @date: 2020/8/21 19:09
 * @description:
 */
public class BreakDao extends BaseDao {

    //创建报损单
    public int addSaleStock(BreakageInfo pvo) {
        return alterTable("INSERT bsd VALUES('" + pvo.getId() + "','" + new java.sql.Date(pvo.getCreatDate().getTime()) + "'," + pvo.getUserId() + ",'" + pvo.getUserName() + "','" + pvo.getRemark() + "')");
    }

    //删除报损单
    public int deleteSaleStock(String pid) {
        //删除订单商品
        alterTable("delete from bsdsp where djid = '" + pid + "'");
        return alterTable("DELETE from bsd where djid = '" + pid + "'");
    }

    //添加商品
    public int addSaleStockPro(BreakProInfo cvo) {
        return alterTable("INSERT bsdsp VALUES(DEFAULT,'" + cvo.getDjId() + "','" + cvo.getProId() + "','" + cvo.getProName() + "','" + cvo.getUnit() + "','" + cvo.getProModel() + "','" + cvo.getPrice() + "'," + cvo.getProNum() + "," + cvo.getTotalPrice() + ")");
    }
    ...
    public BreakageInfo querySaleStockById(String pid) {
        ...
        try (
                ...
                PreparedStatement pst = con.prepareStatement("SELECT * FROM bsd where djid ='" + pid + "'");
    ...
    public ArrayList<BreakageInfo> queryAllSaleStock() {
        ...
        try (
                Connection con = openCon();
                PreparedStatement pst = con.prepareStatement("SELECT * FROM bsd ");
    ...
    public ArrayList<BreakageInfo> queryDatetoDateByPage(Date d1, Date d2, SplitePage sp) {
        ...
        try (
                Connection con = openCon();
                PreparedStatement pst = con.prepareStatement("SELECT * FROM bsd WHERE riqi BETWEEN  ? and ? LIMIT " + start + "," + sp.getPageRows());
    ...
    public int queryAllRowDatetoDate(Date d1, Date d2) {
        ...
        try (
                Connection con = openCon();
                PreparedStatement pst = con.prepareStatement("SELECT count(djid) FROM bsd WHERE riqi BETWEEN  ? and ?");
    ...
    public int queryAllRows() {
        ...
        try (
                Connection con = openCon();
                PreparedStatement pst = con.prepareStatement("SELECT COUNT(djid) from bsd");
                ResultSet rs = pst.executeQuery();
        ) {
            if (rs.next()) {
                i = rs.getInt(1);
    ...
    public ArrayList<BreakageInfo> querySaleStockByPage(SplitePage sp) {
        ...
        try (
                Connection con = openCon();
                PreparedStatement pst = con.prepareStatement("SELECT * from bsd LIMIT " + (sp.getCurrentPage() - 1) * sp.getPageRows() + "," + sp.getPageRows());
                ResultSet rs = pst.executeQuery();
        ) {
    ...
    public ArrayList<BreakProInfo> queryProBySaleStockId(String pid) {
        ...
        try (
                Connection con = openCon();
                PreparedStatement pst = con.prepareStatement("SELECT * from bsdsp where djid = ?");
